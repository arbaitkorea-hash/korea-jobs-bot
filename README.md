  ...
  app/
    ...
    (admin)/admin/         админка, свой root layout (не индексируется)
      ...
      (dashboard)/
        ...
        rates/              курсы валют для пересчёта цены
...
Цена работы задаётся в одной валюте — той, в которой её назначил художник
(в текущем каталоге это рубли). Посетителю она показывается в валюте его
страны: Корея — воны, Россия и соседи — рубли, остальные — доллары. Страну
даёт geo-заголовок хостинга, она кладётся в куку в `src/proxy.ts`, а подмену
делает клиентский компонент `Price` — так страницы остаются статическими и
кешируемыми, а без JavaScript цена всё равно видна в валюте языка страницы.

Пересчитанные суммы помечены знаком «≈», округлены и подписаны исходной ценой:
продаётся работа за ту сумму, которую назначил художник. Курсы задаются руками
в `/admin/rates` — цена картины не должна прыгать вслед за биржей. Если курс не
задан, пересчёта не происходит: посетитель видит исходную цену.
